...
2. CC Switch で `設定 → 一般 → Codex アプリ拡張 → 直接切替時に公式ログインを保持` をオンにする（**デフォルトはオフ**）。ルーティング接管中は、このスイッチに関係なく公式ログインが常に保持されます。
3. そのサードパーティプロバイダーに `ルーティングが必要` バッジが付いている場合（Chat Completions プロトコル。たとえば SiliconFlow、ModelScope）は、ローカルルーティングを有効化し、Codex のルーティングをオンにする。DeepSeek、Kimi、MiniMax などネイティブ直結のプリセットでは、この手順は不要です。
4. Codex を完全に終了して再起動する。

オンにすると、CC Switch はサードパーティプロバイダーへ切り替える際に `~/.codex/auth.json` 内の公式ログイン状態を保持し、サードパーティの Key を `config.toml` へ書き込みます。これにより、デスクトップアプリは引き続き公式ログイン ID を認識してゲーティングを通すため、設定したカスタムモデルがセレクタに再び表示されます。**保持された公式 Token がサードパーティへ送られることはありません**——サードパーティのモデルリクエストは引き続き、設定した Key で送られます（ルーティング接管を有効にしている場合はローカルルーティング経由で転送されます）。

> 詳細な図入り手順: [サードパーティ API 利用時に Codex のリモート操作と公式プラグインを保持する](./codex-official-auth-preservation-guide-ja.md)

## それでも見えない場合は

- **スイッチがオンか確認する**: このスイッチはデフォルトでオフで、オフのままルーティング接管なしでサードパーティへ切り替える（直接切替）と `auth.json` が削除されます。多くの人は初めてサードパーティへ切り替えたときにこうして公式ログイン状態を失っており、その結果見えなくなっています——上記の手順 1、2 に従って操作してください。
