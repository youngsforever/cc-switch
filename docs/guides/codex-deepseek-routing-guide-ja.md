# Codex で Chat 形式 API を使う: ローカルルーティングガイドと DeepSeek 移行の説明

> 対象バージョン: CC Switch 3.20.4 以降。本記事はリポジトリ内のドキュメントとコードをもとに整理しています。スクリーンショットは実際の API Key やアカウント残高が漏れないよう匿名化したサンプルデータで生成しています。
>
> **DeepSeek ユーザーの多くは、もう本記事を必要としません**: 3.19.1 から DeepSeek プリセットはネイティブ Responses による直結に変わり、`deepseek-v4-pro` も直結できるようになりました。本記事は現在、主に 2 種類の読者に向けたものです。Chat 形式のままのプロバイダー（SiliconFlow、ModelScope など）を使う人と、3.19.1 より前に保存され Chat のままになっている DeepSeek の旧カードを使う人です。まず次のセクションで、自分がどのケースに当てはまるかを確認してください。

## まず、このガイドが今も必要か確認する

判別方法は 1 つだけです。Codex プロバイダーカードに `ルーティングが必要` バッジが付いているかを見てください。

![Codex プロバイダー一覧の「ルーティングが必要」マーク](../images/codex-deepseek-routing/01-codex-providers-require-routing.png)
...
このバッジはプロバイダー保存時に記録された API 形式で決まるため、CC Switch をアップグレードしても既存プロバイダーの挙動は**変わりません**。DeepSeek に関しては、次の 3 つのケースがあります。
...
| 3.19.1 以降にプリセットから新規作成した DeepSeek | 不要 | `api.deepseek.com` へ直結し、DeepSeek 公式のモデルカタログを取得します。3.20.4 からプリセットのデフォルトモデルは `deepseek-flash`（V4.1 Flash）です |
| `deepseek-v4-pro` を使いたい | 不要 | DeepSeek は 2026 年 8 月にこのモデルの Codex 連携を開放したため、直結で使えます。プリセットのモデルメニューにもすでに含まれています |

DeepSeek だけでなく、Kimi、Zhipu GLM、MiniMax、千问AI平台 の Codex プリセットも順次ネイティブ直結に変わりました。現在も Chat 形式のままで本記事が必要なプリセットは、主に SiliconFlow、ModelScope、Tencent Token Plan シリーズ、Baidu Qianfan Coding Plan / Token Plan、StepFun などで、以下の手順はそれらに完全に当てはまります。
...
OpenAI 互換 base URL の書き方は各社でまちまちです。よくあるのは `/v1` 付きのもの（たとえば SiliconFlow の `https://api.siliconflow.cn/v1`）で、より長いパスのものもあります（たとえば Baidu Qianfan Coding Plan の `https://qianfan.baidubce.com/v2/coding`）。Chat API のパスは `/chat/completions` です。CC Switch のプリセットには各社のドキュメントに従った設定がすでに入っているため、まずはプリセットを使い、エンドポイントパスを手で組み立てる必要はありません。
