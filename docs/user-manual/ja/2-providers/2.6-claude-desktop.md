...
モデルマッピングモードでは、リクエスト変換のために CC Switch ローカルルーティングが必要です。Claude Desktop パネルの右上には Claude Desktop ローカルルーティング切り替えが常に表示され、「メインページにルーティング切り替えを表示」設定の影響を受けません（この設定が制御するのは Claude Code、Codex、Gemini CLI、Grok Build ページの切り替えだけです）。
...
他のアプリがローカルルーティングを使用している場合、ローカルルーティングの停止がブロックされることがあります。先に「設定 → ルーティング → ローカルルーティング」で該当アプリのルーティングをオフにしてから、ローカルルーティングを停止してください。
...
- [ローカルルーティングサービス](../4-proxy/4.1-service.md)
- [アプリケーションルーティング](../4-proxy/4.2-routing.md)
