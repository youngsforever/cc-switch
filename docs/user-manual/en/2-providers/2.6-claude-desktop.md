...
Model mapping mode needs CC Switch local routing to convert requests. The Claude Desktop local routing toggle is always shown in the top-right corner of the Claude Desktop panel, regardless of the "Show Routing Toggle on Main Page" setting (that setting only controls the toggle on the Claude Code, Codex, Gemini CLI, and Grok Build pages).
...
If another app is using local routing, stopping local routing may be blocked. First turn off routing for that app in "Settings → Routing → Local Routing", then stop local routing.
...
- [Local Routing Service](../4-proxy/4.1-service.md)
- [App Routing](../4-proxy/4.2-routing.md)
