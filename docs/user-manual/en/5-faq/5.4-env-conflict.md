...
| App | Detection Scope | Common Variables |
|-----|-----------------|------------------|
| Claude Code | Variables starting with `ANTHROPIC` | `ANTHROPIC_API_KEY`, `ANTHROPIC_BASE_URL` |
| Codex | Variables starting with `OPENAI` | `OPENAI_API_KEY` |
| Gemini CLI | Variables starting with `GEMINI` or `GOOGLE_GEMINI` | `GEMINI_API_KEY` |
...
Found X environment variables that may override your configuration
[View Details] [×]
```

## View Conflict Details

Click the "View Details" button to see the details:
...
- Backup location: `~/.cc-switch/backups/env-backup-<timestamp>.json`
...
1. Click the close button (×) on the right side of the warning banner
...
1. Find the backup file: `env-backup-<timestamp>.json` under `~/.cc-switch/backups/`
