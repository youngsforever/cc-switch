...
Skills are supported in the following applications:

- **Claude Code** (the Skills panel on the Claude Desktop page also works on Claude Code)
- **Codex**
- **Gemini CLI**
- **Grok Build**
- **OpenCode**
...
OpenClaw does not support Skills management yet.
...
If you override a configuration directory in Settings, the install directory changes accordingly (MiniMax Code's data directory can be set with the `MINIMAX_DATA_DIR` environment variable, and Pi additionally recognizes `PI_CODING_AGENT_DIR`). The master copy of each skill lives in the directory chosen under "Skill Storage Location" (default `~/.cc-switch/skills/`; see [Storage Location Switch](#storage-location-switch) below) and is synced to each app's directory by symlink or file copy.
...
- Removes the skill from every app's skills directory
- Removes the skill from the storage directory holding the master copy (default `~/.cc-switch/skills/`)
...
Select the target storage location in "Settings → General → Skill Storage Location". The switch **does not lose skill state** — CC Switch smoothly migrates existing skills to the new location.
