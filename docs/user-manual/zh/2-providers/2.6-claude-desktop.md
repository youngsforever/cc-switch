...
模型映射模式需要 CC Switch 本地路由参与请求转换。Claude Desktop 面板的右上角始终显示 Claude Desktop 本地路由开关，不受「在主页面显示本地路由开关」设置的影响（该设置只控制 Claude Code、Codex、Gemini CLI、Grok Build 页面的开关）。
...
如果其它应用正在使用本地路由，关闭本地路由可能会被阻止。请先到「设置 → 路由 → 本地路由」关闭对应应用的路由，再停止本地路由。
